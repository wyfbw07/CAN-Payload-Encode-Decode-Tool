...
void DbcParser::loadAndParseFromFile(std::istream& in) {
	...
	while (std::getline(in,line)) {
        ...
        // Look for signal value descriptions
            ...
            // Skip the line for uninterested data
...
void DbcParser::decode(uint32_t msgId, std::string payload) {
...
void DbcParser::decodeSignalOnRequest(uint32_t msgId, std::string payload, std::string msgName) {
    std::unordered_map<std::string, double> result;
    result = messageLibrary[msgId].decode(payload);
